...
- Регистрация новых пользователей c рассылкой события `user.registered` через Kafka
...
**Важно**: вместо `EXAMPLE` необходимо вставить в верхнем регистре название добавленного в БД приложения
...
- После успешной смены все существующие сессии пользователя удаляются
...
При смене почты пользователя сервис отправляет событие в Kafka
